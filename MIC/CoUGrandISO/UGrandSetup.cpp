...
CUGrandSetup::CUGrandSetup(CWnd* pParent, char *inifile)
	: CDialog(CUGrandSetup::IDD, pParent)
{
//	TRACE("CUGrandSetup::CTR\n");
	m_pParent = pParent;
	strncpy(m_szIniFile, inifile, sizeof(m_szIniFile));
	m_pList = new CListCtrlStyled();
	//{{AFX_DATA_INIT(CUGrandSetup)
		// NOTE: the ClassWizard will add member initialization here
	//}}AFX_DATA_INIT
}

CUGrandSetup::~CUGrandSetup()
{
//	TRACE("CUGrandSetup::CTR\n");
	delete m_pList;
}
...
void CUGrandSetup::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CUGrandSetup)
	DDX_Control(pDX, IDC_BUTTON_EDITSELECTION, m_cEditSelected);
	DDX_Control(pDX, IDC_LIST_SET, *m_pList);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CUGrandSetup, CDialog)
	//{{AFX_MSG_MAP(CUGrandSetup)
...
void CUGrandSetup::OnButtonHide()
{
	ShowWindow(SW_HIDE);
}

void CUGrandSetup::AddSetParam(char*szDesc, char*szIniName, char*szSetting, char*szCommand,unsigned uLength)
{
...
void CUGrandSetup::OnButtonLoadDefault()
{
	CWaitCursor wait;
...
//	{
//		while ((pszDescription = cUGrandSetupData.GetNewNext())!=NULL)
//		{
//			pszIniName = cUGrandSetupData.GetNewNext();
//			pszDefault = cUGrandSetupData.GetNewNext();
//			pszCommand = cUGrandSetupData.GetNewNext();
//			GetPrivateProfileString(m_szName,pszIniName,"",szTemp,sizeof(szTemp),m_szIniFile);
//			AddSetParam(pszDescription,pszIniName,szTemp,pszCommand,strlen(pszDefault));
//			AddSetParam(pszDescription,pszIniName,szTemp,pszCommand,strlen(pszDefault));
//		}
...
void CUGrandSetup::CurrentSetParam(char*szIniName,char*szCurrent,bool bMatch, bool bTextOnly)
{
	//find item's row
	CString cString;
	m_pList->SetRedraw(false);
...
/////////////////////////////////////////////////////////////////////////////
// CUGrandSetup message handlers

BOOL CUGrandSetup::OnInitDialog() 
{
...
void CUGrandSetup::SaveSetup()
{
	SaveAll(true);
}

void CUGrandSetup::OnButtonSaveall() 
{
	SaveAll(false);
}

void CUGrandSetup::SaveAll(bool bDoingConfig)
{
	CWaitCursor wait;
...
void CUGrandSetup::OnDblclkListSet(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
	OnButtonEditselection();	
	*pResult = 0;
}

void CUGrandSetup::OnItemchangedListSet(NMHDR* /*pNMHDR*/, LRESULT* pResult) 
{
