...
{
	m_pPSParent->m_pDataWatchDlg = NULL;
}
...
{
	m_pPSParent->RestartAutoCloseTimer();
...
void CDataWatchDlg::OnNewH1Limits(WPARAM wParam, LPARAM lParam)
{
	...
	jTime.Init(m_uBaseTime+lParam);//+m_wnd1Chart.GetHorzRange());
	cTemp.Format("%s %s",jTime.m_pDate,jTime.m_pTime);
...
void CDataWatchDlg::OnNewH2Limits(WPARAM wParam, LPARAM lParam)
{
	...
	jTime.Init(m_uBaseTime+lParam);//wParam+m_wnd2Chart.GetHorzRange());
	cTemp.Format("%s %s",jTime.m_pDate,jTime.m_pTime);
...
void CDataWatchDlg::OnNewH3Limits(WPARAM wParam, LPARAM lParam)
{
	...
	jTime.Init(m_uBaseTime+lParam);//wParam+m_wnd3Chart.GetHorzRange());
	cTemp.Format("%s %s",jTime.m_pDate,jTime.m_pTime);
...
{
	// TODO: Add your message handler code here and/or call default
	m_pPSParent->m_bDataWatchDlgOpen = false;
...
void CDataWatchDlg::OnButtonContract() 
{
	...
	if (m_uiMultiplier == 1) m_cButtonContract.EnableWindow(FALSE);
